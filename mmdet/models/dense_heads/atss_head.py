...
class ATSSHead(AnchorHead):
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, centerness, labels,
        ...
        centerness = centerness.permute(0, 2, 3, 1).reshape(-1).contiguous()
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, centerness, anchors in zip(
            ...
            scores = cls_score.permute(1, 2, 0).reshape(
                -1, self.cls_out_channels).sigmoid().contiguous()
            bbox_pred = bbox_pred.permute(1, 2, 0).reshape(-1, 4).contiguous()
            centerness = centerness.permute(1, 2, 0).reshape(-1).sigmoid().contiguous()
